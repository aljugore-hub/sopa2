...
var posX=-1;var posY=-1;var posXEnd=-1;var posYEnd=-1;var clicking=false; var currentWord="";var pL;var iL=0;var indexSolved=-1;var xp=0;var yp=0;
function initAct(){
...
$("#s_pointer").hide();$("#s_pointer").css("position","absolute");$("#s_pointer").css("z-index",50);$("#s_pointer").attr("height",$("#ardoraActSel").attr("height"));$("#s_pointer").attr("width",$("#ardoraActSel").attr("width"));$("#horizontal").attr("x2",$("#ardoraActSel").attr("width"));$("#vertical").attr("y2",$("#ardoraActSel").attr("height"));
var posimaAct = $("#ardoraActSel").position();$("#s_pointer").css($("#ardoraActSel").position());$("body").mousemove(function(e) {$("#s_pointer").hide();});$("#ardoraActSel").attr("tabindex", "0");
$("#ardoraActSel").keydown(function(e) {if (e.which !=9 && e.which != 37 && e.which != 39 && e.which != 39) {drawcross();}; if (e.which==37) {if (xp>1) {xp--;xp--;drawcross();}};
if (e.which==39) {if (xp < $("#ardoraActSel").width() - 1) {xp++;xp++;drawcross();}};if (e.which == 38) {if (yp > 1) {yp--;yp--;drawcross();}}if (e.which == 40) {if (yp < $("#ardoraActSel").height() - 1) {yp++;yp++;drawcross();}}
if (e.which == 13) {if (!clicking){if (posX == -1 && posY == -1) {posX = Math.floor((xp / sizeCell)) * sizeCell;posY = Math.floor((yp / sizeCell)) * sizeCell;}; if (posX > sizeCell * (numCols - 1) || posY > sizeCell * (numRows - 1)) {posX = -1;posY = -1;clicking = false;} else {clicking = true;}}
else{posXEnd = (Math.floor(xp / sizeCell)) * sizeCell;posYEnd = (Math.floor(yp / sizeCell)) * sizeCell;doClick(posXEnd, posYEnd);makeCurrentWord(posX, posY, posXEnd, posYEnd);checkWord(posX, posY, posXEnd, posYEnd);clicking = false;posX=-1;posY=-1;}}
if (clicking){posXEnd = (Math.floor(xp / sizeCell)) * sizeCell;posYEnd = (Math.floor(yp / sizeCell)) * sizeCell;doClick(posXEnd, posYEnd);makeCurrentWord(posX, posY, posXEnd, posYEnd);}});
}
...
if (indexSolved==words.length-1){$("#ardoraActSel").unbind();score = scoreInc;successes=1;$("#ardoraActCanvas").attr("aria-label",messageOk); showMessage("Ok");}}
...
newDiv = $('<div id="idDiv'+j.toString()+'"></div>');
newDiv.css({height: (sizeCell-5).toString()+"px",width: (sizeCell-5).toString()+"px",position: "absolute",padding: "0px",
margin: "0px",left: (numCols * sizeCell + 10).toString()+"px",top: (lineWord - sizeCell + 5).toString()+"px"});$(newDiv).css("z-index" , "50"); $("#ardoraAct").append(newDiv);
context.fill();context.stroke();context.restore();
...
function defMediaDivs(){
}
function defAudioDivs(){
}
function paintImaDivs(x,y,imageSrc,nalt){
while(y+450>$("#ardoraActSel").height()){y=y-1};while(x+600>$("#ardoraActSel").width()){x=x-1};
$("#ardoraActSel").attr("role","img");$("#ardoraActSel").attr("aria-label",nalt);
var canvas = document.getElementById("ardoraActSel");var context = canvas.getContext("2d");
var imageObj = new Image();imageObj.alt=nalt;imageObj.src = imageSrc;imageObj.onload = function() {context.drawImage(imageObj,x,y);};}
